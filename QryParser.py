...
class QryParser:
    ...
    def __createOperator(operatorName):
        ...
        if operatorNameLowerCase == '#or':
            ...
            try:
                distance = int(operatorNameLowerCase.split('/')[1])
            except:
                QryParser.__syntaxError(
                    'Invalid NEAR operator syntax: ' + operatorName)
            operator = QryIopNear(distance)
            ...
            try:
                ...
                QryParser.__syntaxError(
                    'Invalid NEAR operator syntax: ' + operatorName)
            operator = QryIopWindow(distance)
        else:
    ...
    def parseString(queryString):
	    ...
	    # STUDENTS:
            ...
            # Check if the current operator is a weighted operator
            if isinstance(queryTree, QrySopWsum):       # Weight
                # Pop the weight
