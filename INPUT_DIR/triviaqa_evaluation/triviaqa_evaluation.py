...
2025-11-11: Modified to (mostly) only consider questions in the
            prediction_file. Missing questions are ignored.
"""
...
import sys
import argparse
import triviaqa_evaluation_dataset_utils
import triviaqa_evaluation_utils
...
#                print(message, file=sys.stderr)
        ...
        if not mute:
            print(f"qid: {qid}, em={em}:", prediction, ground_truths)
    ...
    qid_list = [1] * len(predicted_answers)
    ground_truth = [1] * len(predicted_answers)
#    exact_match = 100.0 * exact_match / len(qid_list)
...
if __name__ == '__main__':
    ...
    dataset_json = triviaqa_evaluation_dataset_utils.read_triviaqa_data(args.dataset_file)
    if dataset_json['Version'] != expected_version:
        print('Evaluation expects v-{} , but got dataset with v-{}'.format(expected_version,dataset_json['Version']),
              file=sys.stderr)
    key_to_ground_truth = triviaqa_evaluation_dataset_utils.get_key_to_ground_truth(dataset_json)
    predictions = triviaqa_evaluation_utils.read_json(args.prediction_file)
